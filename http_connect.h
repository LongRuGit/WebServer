...
public:
    //文件名的最大长度
    static const int FILENAME_LEN=250;
    //读缓冲去的大小
    ...
    enum CHECK_STATE
    {
        CHECK_STATE_REQUESTLINE=0,CHECK_STATE_HEADER,CHECK_STATE_CONNECT
    };
    //服务器处理HTTP请求的可能结果
    enum HTTP_CODE
    {
...
public:
    //所有的socket上的事件都被注册到同一个epoll内核事件表中,所以将epoll文件描述符设置为静态
    static int m_epollfd;
    //统计用户的数量
    static int m_user_count;
private:
    //该HTTP连接的socket和对方的socket连接
    int m_sockfd;
    ...
    char * m_file_address;
    //目录文件的状态,通过它我们可以判断文件是否存在,是否为目录,是都可读,并获取文件大小等信息
